...
export function getUserData(xml) {
  const tableData = [];
  tableData.push(USER_TABLE_HEADERS);

  const xmlResponse = new JSDOM(xml);

  const user = xmlResponse.window.document.querySelector('useasync fufr');
...
export function mapAccountPremium(code) {
  return Number(code) === 0 ? 'free' : 'premium';
}

export function mapAccountVisibility(code) {
  switch (Number(code)) {
    case 0:
      return 'public';
    case 1:
...
export async function openInBrowser(id) {
  await open(`https://pastebin.com/${id}`);
}
